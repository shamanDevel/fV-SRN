...
void checkAdjoint(const Vector_t& x, const ForwardFunction<Vector_t, TmpStorage_t>& forward, 
    ...
    double epsilon = 1e-5, double precision = 1e-6, double margin = 1e-10)
{
    int n = x.size();
    ...
    for (int trial = 0; trial < numDirections + m; ++trial)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            Vector_t x2 = x;
            x2[i] += epsilon;
            const double adjZAnalytical = adjZ[i] - adjZCopy[i];
            Vector_t y2 = forward(x2, nullptr);
            Vector_t gradYNumerical = (y2 - ref) / epsilon;
            const double adjZNumerical = gradYNumerical.dot(adjE);
            CHECK(adjZAnalytical == Approx(adjZNumerical).epsilon(precision).margin(margin));
        }
        //for (int i = 0; i<n; ++i)
        //{
        //    INFO("x': i=" << i);
...
template<typename real_t, typename Vector_t>
inline void checkDerivative(const Vector_t& x, const ScalarFunction<real_t, Vector_t>& fun, 
    const ScalarFunctionDerivative<Vector_t>& derivative,
    double epsilon = 1e-5, double precision = 1e-4)
{
    int n = x.size();
