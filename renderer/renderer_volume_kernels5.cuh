...
__global__ void EvaluateNoBatchesWithGradientAndCurvature(
	dim3 virtual_size,
	...
	kernel::Tensor2RW<real_t> curvatureOutput)
{
...
#endif
	{
		...
		real2 curvature = volume.evalCurvature(position, direction, resultFromEval, batch);
		if (valid) {
			densitiesOutput[b][0] = density;
			gradientsOutput[b][0] = gradient.x;
			gradientsOutput[b][1] = gradient.y;
			gradientsOutput[b][2] = gradient.z;
			curvatureOutput[b][0] = curvature.x;
			curvatureOutput[b][1] = curvature.y;
		}
